...
### 2026/09/25 檢定術科與頁首導覽（016）

先於既有 Supabase 專案套用 `016_exam_practical.sql` 一次，再發布前端。此升級只新增可留空的術科欄位、更新受保護登分與公開快照 RPC，不變更現有分數、名單、公布狀態或權限。舊裁判頁省略術科欄位時，後端保留原術科結果。

術科為「完成／未完成」，未登錄以 null 表示，不預設未完成。學科至少 80 分且術科完成才通過；缺少任一項顯示待確認。允許先存學科或術科，已登錄值不能以空白清除。更正需原因、版本核對，重送使用相同請求識別碼。沿用手動統一公布：本次公布所有已登學科者的學科、術科快照，只有術科而沒有學科者暫不列入；缺術科的公開結果為待確認。家長只取得遮罩姓名與狀態，不取得實際分數。未公布草稿不影響家長端。

四頁共用 EventNavigation，連到 https://ttra-2026-rules-m913aib.gamma.site 。手機頁首採標誌一列、等寬導覽按鈕一列；家長頁不加入工作台入口。
